...
void initMap(Map *map, const int capacity) {
    map->entries = (MapEntry *) malloc(sizeof(MapEntry) * capacity);
    map->size = 0;
    map->capacity = capacity;
}

void put(Map *map, const int key, const int value) {
...
Interval *getMinimumPriceIntervals(const Interval *priceIntervals, int n, int *resultSize) {
    Map map;
    initMap(&map, 100);
